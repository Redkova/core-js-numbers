...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const moduleV1 = Math.sqrt(x1 * x1 + y1 * y1);
  const moduleV2 = Math.sqrt(x2 * x2 + y2 * y2);
  const vectorModules = moduleV1 * moduleV2;
  const inversCos = Math.acos(scalarProductOfVectors / vectorModules);
  return inversCos;
}
...
function getParallelepipedDiagonal(a, b, c) {
  const diagonalLength = Math.sqrt(a * a + b * b + c * c);
...
function getSumOfDigits(num) {
  let sum = 0;
  let absoluteValueOfNum = Math.abs(num);
  while (absoluteValueOfNum > 0) {
    const lastDigit = absoluteValueOfNum % 10;
    absoluteValueOfNum = Math.floor(absoluteValueOfNum / 10);
    sum += lastDigit;
  }
  return sum;
}
